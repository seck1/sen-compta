...
<div class="page-header">
    ...
    <div style="display:flex;gap:10px;align-items:center">
        <form method="get" action="" style="display:flex;gap:8px;align-items:center">
            ...
            <select name="mois" onchange="this.form.submit()" style="padding:8px 12px;border-radius:8px;border:1px solid var(--border);font-size:17px;background:#fff">
                <?php for($m=1;$m<=12;$m++): ?>
...
<div style="background:linear-gradient(135deg,var(--navy-dark),var(--navy-light));border-radius:16px;padding:24px 28px;margin-bottom:24px;color:#fff;display:flex;align-items:center;justify-content:space-between">
    <div>
        <div style="font-size:16px;opacity:.7;margin-bottom:4px;text-transform:uppercase;letter-spacing:1px">Rapport de gestion</div>
        <div style="font-size:22px;font-weight:700"><?= $mois_labels[$mois_courant] ?> <?= $exercice ?></div>
        <div style="font-size:16px;opacity:.6;margin-top:4px">Généré le <?= date('d/m/Y') ?></div>
    </div>
    <div style="text-align:right">
        <div style="font-size:16px;opacity:.7">Dossier</div>
        <div style="font-size:19px;font-weight:600"><?= e($entreprise['raison_sociale']) ?></div>
        <div style="font-size:15px;opacity:.5"><?= e($entreprise['forme_juridique']) ?> · <?= e($entreprise['code_dossier']) ?></div>
    </div>
</div>
...
<div style="text-align:center;padding:60px 0;color:var(--text-muted)">
    ...
    <div style="font-size:19px;font-weight:500;margin-bottom:8px">Cliquez sur "Imprimer / PDF" pour générer le rapport complet</div>
    <div style="font-size:16px;opacity:.6">Le rapport s'ouvrira dans un nouvel onglet, prêt à imprimer ou enregistrer en PDF</div>
    <div style="margin-top:24px">
        <a href="<?= APP_URL ?>/dossier/rapport-gestion/export?id=<?= $entreprise['id'] ?>&mois=<?= $mois_courant ?>" target="_blank" class="btn btn-primary" style="font-size:18px;padding:12px 28px">
