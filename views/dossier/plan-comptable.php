...
<?php if ($error ?? null): ?>
<div style="background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.2);border-radius:10px;padding:12px 18px;color:var(--danger);margin-bottom:20px;font-size:14px"><?= e($error) ?></div>
<?php endif; ?>
...
<div style="display:grid;grid-template-columns:1fr 320px;gap:20px;align-items:start">
        ...
        <div class="table-wrap" style="margin-bottom:16px">
            <div class="table-header" style="background:rgba(30,58,95,0.04)">
                <div style="display:flex;align-items:center;gap:12px">
                    <div style="width:34px;height:34px;border-radius:9px;background:var(--ent-color);display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:700;color:white">
                        <?= $classe ?>
                    </div>
                    <div>
                        <div style="font-size:14px;font-weight:600;color:var(--navy-dark)">Classe <?= $classe ?> — <?= $classes[$classe] ?? '' ?></div>
                        <div style="font-size:13px;color:var(--text-muted)"><?= count($comptes_classe) ?> compte<?= count($comptes_classe)>1?'s':'' ?></div>
                    </div>
                </div>
            </div>
            <table>
                ...
                <tr>
                    <td>
                        <code style="background:var(--bg);padding:3px 9px;border-radius:6px;font-size:13px;font-weight:600;color:var(--navy)">
                            <?= e($c['numero']) ?>
                        </code>
                    </td>
                    <td style="font-size:14px"><?= e($c['intitule']) ?></td>
                    <td>
                    ...
                    <td style="text-align:right;font-family:monospace;font-size:14px;color:var(--danger)">
                        <?= $c['mvt_debit']>0 ? number_format($c['mvt_debit'],0,',',' ') : '—' ?>
                    </td>
                    <td style="text-align:right;font-family:monospace;font-size:14px;color:var(--success)">
                        <?= $c['mvt_credit']>0 ? number_format($c['mvt_credit'],0,',',' ') : '—' ?>
                    </td>
                    <td style="text-align:right;font-family:monospace;font-size:14px;font-weight:600;color:<?= $solde>0?'var(--danger)':($solde<0?'var(--success)':'var(--text-muted)') ?>">
                    ...
                    <td style="text-align:center">
                        ...
                        <a href="<?= APP_URL ?>/dossier/plan-comptable/store?action=delete&compte_id=<?= $c['id'] ?>&id=<?= $entreprise['id'] ?>"
                           style="color:var(--danger);font-size:13px;text-decoration:none;opacity:0.5"
    ...
    <div class="card" style="position:sticky;top:80px">
        ...
        <div style="font-size:13px;color:var(--text-muted);line-height:1.7">
            <div style="font-weight:600;color:var(--navy-dark);margin-bottom:8px;font-size:14px">Classes OHADA</div>
