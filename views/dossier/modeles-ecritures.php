...
.modele-icon { width:40px; height:40px; border-radius:8px; background:var(--navy-dark); display:flex; align-items:center; justify-content:center; flex-shrink:0; color:#fff; font-size:13px; }
.modele-info { flex:1 }
.modele-nom { font-weight:700; font-size:14px; color:var(--navy-dark); }
.modele-desc { font-size:13px; color:var(--text-muted); margin-top:2px; }
...
.ligne-row input { padding:7px 10px; border:1px solid var(--border); border-radius:6px; font-size:14px; width:100%; box-sizing:border-box; }
.btn-add-ligne { background:none; border:1px dashed var(--border); border-radius:6px; padding:8px; width:100%; font-size:14px; color:var(--text-muted); cursor:pointer; margin-top:4px; }
...
<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:12px 16px;margin-bottom:20px;color:#166534;font-size:14px">Modèle enregistré.</div>
<?php endif; ?>
<?php if(isset($_GET['error'])): ?>
<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:12px 16px;margin-bottom:20px;color:#991b1b;font-size:14px">Veuillez renseigner un nom et au moins une ligne avec un numéro de compte.</div>
<?php endif; ?>

<?php if(empty($modeles)): ?>
<div class="card" style="padding:56px;text-align:center;color:var(--text-muted)">
    <div style="font-size:40px;margin-bottom:16px">📋</div>
    <div style="font-weight:700;font-size:13px;margin-bottom:8px">Aucun modèle d'écriture</div>
    <div style="font-size:14px;margin-bottom:20px">Créez des modèles pour loyer, salaires, charges sociales… et passez vos écritures en 1 clic.</div>
...
<div class="modele-card">
    ...
    <div style="display:flex;flex-direction:column;gap:6px;flex-shrink:0">
        <button onclick="appliquerModele(<?= $m['id'] ?>, '<?= e(addslashes($m['nom'])) ?>')" class="btn btn-primary" style="font-size:13px;padding:6px 12px">⚡ Appliquer</button>
        <button onclick="editerModele(<?= $m['id'] ?>)" class="btn btn-secondary" style="font-size:13px;padding:6px 12px">✏️ Éditer</button>
        <button onclick="supprimerModele(<?= $m['id'] ?>)" style="background:none;border:1px solid #fecaca;border-radius:6px;color:#dc2626;font-size:13px;padding:6px 12px;cursor:pointer">🗑️</button>
    </div>
</div>
<?php endforeach; ?>
</div>
...
<div style="background:#fff;border-radius:14px;width:760px;max-width:95vw;max-height:90vh;overflow-y:auto;padding:28px">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
        <div style="font-size:13px;font-weight:700;color:var(--navy-dark)" id="formTitre">Nouveau modèle d'écriture</div>
    ...
    <form method="post" action="<?= APP_URL ?>/dossier/modeles/store" id="formModele">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 120px;gap:12px;margin-bottom:16px">
            <div>
                <label style="font-size:13px;font-weight:600;color:var(--text-muted);display:block;margin-bottom:4px">Nom du modèle *</label>
                <input type="text" name="nom" id="fm_nom" required placeholder="Ex: Loyer mensuel" style="width:100%;padding:8px 12px;border:1px solid var(--border);border-radius:6px;font-size:14px;box-sizing:border-box">
            </div>
            <div>
                <label style="font-size:13px;font-weight:600;color:var(--text-muted);display:block;margin-bottom:4px">Description</label>
                <input type="text" name="description" id="fm_desc" placeholder="Optionnel" style="width:100%;padding:8px 12px;border:1px solid var(--border);border-radius:6px;font-size:14px;box-sizing:border-box">
            </div>
            <div>
                <label style="font-size:13px;font-weight:600;color:var(--text-muted);display:block;margin-bottom:4px">Journal</label>
                <input type="text" name="journal_code" id="fm_journal" value="OD" maxlength="5" style="width:100%;padding:8px 12px;border:1px solid var(--border);border-radius:6px;font-size:14px;box-sizing:border-box;text-transform:uppercase">
            </div>
        </div>

        <div style="margin-bottom:10px">
            <div style="font-size:13px;font-weight:700;color:var(--navy-dark);margin-bottom:8px;text-transform:uppercase;letter-spacing:.5px">Lignes d'écriture</div>
...
<div style="background:#fff;border-radius:14px;width:420px;padding:28px">
    <div style="font-size:14px;font-weight:700;margin-bottom:6px;color:var(--navy-dark)">Appliquer le modèle</div>
    <div style="font-size:14px;color:var(--text-muted);margin-bottom:20px" id="appliquerNom"></div>
    <div style="margin-bottom:14px">
        <label style="font-size:13px;font-weight:600;color:var(--text-muted);display:block;margin-bottom:4px">Date de l'écriture</label>
        <input type="date" id="appliquerDate" value="<?= date('Y-m-d') ?>" style="width:100%;padding:8px 12px;border:1px solid var(--border);border-radius:6px;font-size:14px;box-sizing:border-box">
    </div>
    <div style="margin-bottom:20px">
        <label style="font-size:13px;font-weight:600;color:var(--text-muted);display:block;margin-bottom:4px">Libellé (optionnel)</label>
        <input type="text" id="appliquerLibelle" placeholder="Laissez vide pour utiliser le nom du modèle" style="width:100%;padding:8px 12px;border:1px solid var(--border);border-radius:6px;font-size:14px;box-sizing:border-box">
    </div>
    <div id="appliquerResult" style="display:none;padding:10px;border-radius:6px;font-size:14px;margin-bottom:16px"></div>
...
function ajouterLigne(c, l, d, cr) {
    ...
    btn.type = 'button';
    btn.textContent = '×';
    btn.style.cssText = 'background:#fef2f2;border:1px solid #fecaca;border-radius:4px;color:#dc2626;cursor:pointer;font-size:14px;width:32px;height:32px;flex-shrink:0';
