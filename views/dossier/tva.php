...
function tva_row(string $label, float $montant, string $note = '', bool $bold = false, string $color = ''): void {
    $style_label = 'font-size:16px;' . ($bold ? 'font-weight:600;' : '') . ($color ? "color:$color;" : '');
    $style_val   = 'font-family:monospace;font-size:16px;white-space:nowrap;' . ($bold ? 'font-weight:600;' : '') . ($color ? "color:$color;" : '');
    ...
    if ($note) echo ' <span style="font-size:17px;font-weight:400;color:var(--text-muted);margin-left:6px">' . htmlspecialchars($note) . '</span>';
    echo '</span>';
...
<div style="max-width:900px">
    ...
    <div style="background:rgba(59,130,246,0.05);border:1px solid rgba(59,130,246,0.18);border-radius:14px;padding:24px;margin-bottom:16px">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0 2px">
            <span style="font-size:16px;font-weight:600;color:#1d4ed8">= Sous-total TVA brute collectée</span>
            <span style="font-family:monospace;font-size:17px;font-weight:700;color:#1d4ed8"><?= formatMontant($tva['tva_collectee']) ?></span>
        </div>
        ...
        <div style="background:rgba(239,68,68,0.07);border:1px solid rgba(239,68,68,0.18);border-radius:8px;padding:10px 14px;margin-top:12px;display:flex;justify-content:space-between;align-items:center">
            <span style="font-size:16px;color:#dc2626">
                (-) Retenue à la source — 4445
                <span style="font-size:17px;font-weight:400;margin-left:6px">(30% marchés publics)</span>
            </span>
            <span style="font-family:monospace;font-size:16px;font-weight:600;color:#dc2626">- <?= formatMontant($tva['retenue_source']) ?></span>
        </div>
        <?php endif; ?>
    ...
    <div style="border:2px solid <?= $tva['tva_a_payer'] > 0 ? 'rgba(239,68,68,0.35)' : 'rgba(34,197,94,0.35)' ?>;border-radius:14px;padding:28px;margin-bottom:20px;<?= $tva['tva_a_payer'] > 0 ? 'background:rgba(239,68,68,0.04)' : 'background:rgba(34,197,94,0.04)' ?>">
        ...
        <div style="max-width:520px">
            <div style="display:flex;justify-content:space-between;padding:9px 0;border-bottom:1px solid var(--border);font-size:16px">
            ...
            <div style="display:flex;justify-content:space-between;padding:9px 0;border-bottom:1px solid var(--border);font-size:16px;color:var(--text-muted)">
                <span>(-) Crédit de TVA antérieur</span>
                <span style="font-family:monospace">- <?= formatMontant($tva['credit_anterieur']) ?></span>
            </div>
            <?php endif; ?>
            <div style="display:flex;justify-content:space-between;padding:10px 0;font-size:17px;font-weight:600;border-bottom:1px solid var(--border)">
                <span>= TVA nette</span>
        ...
        <div style="text-align:center;margin-top:24px">
            <?php if($tva['tva_a_payer'] > 0): ?>
                <div style="font-size:18px;text-transform:uppercase;letter-spacing:1.5px;color:#dc2626;font-weight:700;margin-bottom:8px">TVA A PAYER</div>
        ...
        <?php if($tva['ca_ht'] > 0): ?>
        <div style="margin-top:20px;padding-top:16px;border-top:1px solid var(--border);font-size:18px;color:var(--text-muted);text-align:center">
    ...
    <div class="card" style="text-align:center;padding:56px 48px;color:var(--text-muted)">
        ...
        <p style="font-size:17px;margin:0">Sélectionnez une période et cliquez sur <strong>Calculer</strong> pour lancer le calcul de la TVA.</p>
        <p style="font-size:18px;margin:8px 0 0;color:var(--text-muted)">Taux normal : 18% — Retenue à la source marchés publics : 30% — Comptes OHADA 4431/4432/4441/4442/4443/4445</p>
    </div>
    <?php endif; ?>
    ...
    <div style="margin-top:32px">
        <div style="font-size:17px;font-weight:700;text-transform:uppercase;letter-spacing:1.2px;color:var(--text-muted);margin-bottom:14px;display:flex;align-items:center;gap:8px">
            ...
            <table style="width:100%;border-collapse:collapse;font-size:16px">
                ...
                <tr style="border-bottom:1px solid var(--border);<?= $isCurrent ? 'background:rgba(59,130,246,0.04)' : '' ?>">
                    ...
                    <td style="padding:11px 16px;text-align:center">
                        <span style="display:inline-block;padding:3px 10px;border-radius:20px;font-size:17px;font-weight:600;background:<?= $statutConfig['bg'] ?>;color:<?= $statutConfig['color'] ?>">
                            <?= $statutConfig['label'] ?>
                        </span>
                    </td>
                    <td style="padding:11px 16px;text-align:center;color:var(--text-muted);font-size:18px">
                        <?php if($decl['statut'] === 'paye' && $decl['date_paiement']): ?>
                            <?= date('d/m/Y', strtotime($decl['date_paiement'])) ?>
                            <?php if($decl['reference_paiement']): ?>
                            <div style="font-size:17px;color:var(--text-muted)"><?= e($decl['reference_paiement']) ?></div>
                            <?php endif; ?>
                        <?php else: ?>
                ...
                <tr id="<?= $modalId ?>" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:1000;align-items:center;justify-content:center">
                    <td colspan="8" style="padding:0;border:none">
                        <div style="background:var(--bg-card);border-radius:16px;padding:28px;width:420px;box-shadow:0 20px 60px rgba(0,0,0,0.3)">
                            <div style="font-size:18px;font-weight:700;margin-bottom:4px">Enregistrer le paiement</div>
                            ...
                            <form method="POST" action="<?= APP_URL ?>/dossier/tva/payer">
                                ...
                                <div style="display:flex;gap:10px;justify-content:flex-end">
                                    <button type="button" onclick="document.getElementById('<?= $modalId ?>').style.display='none'"
                                            style="padding:8px 16px;border-radius:8px;border:1px solid var(--border);background:transparent;cursor:pointer;font-size:16px;color:var(--text-muted)">
                                        Annuler
                                    </button>
                                    <button type="submit"
