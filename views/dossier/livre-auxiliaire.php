...
<div style="max-width:1100px">
    <div class="page-header">
        ...
        <div style="display:flex;gap:10px;align-items:center">
            <a href="<?= APP_URL ?>/dossier/livre-auxiliaire?id=<?= $entreprise['id'] ?>&type=client<?= $tiers_id ? '&tiers_id='.$tiers_id : '' ?>&date_debut=<?= $date_debut ?>&date_fin=<?= $date_fin ?>"
               style="padding:7px 14px;border-radius:8px;font-size:14px;font-weight:600;text-decoration:none;<?= $type==='client' ? 'background:var(--accent);color:#fff' : 'background:var(--bg-secondary);color:var(--text-muted);border:1px solid var(--border)' ?>">
                Clients
            </a>
            <a href="<?= APP_URL ?>/dossier/livre-auxiliaire?id=<?= $entreprise['id'] ?>&type=fournisseur<?= $tiers_id ? '&tiers_id='.$tiers_id : '' ?>&date_debut=<?= $date_debut ?>&date_fin=<?= $date_fin ?>"
               style="padding:7px 14px;border-radius:8px;font-size:14px;font-weight:600;text-decoration:none;<?= $type==='fournisseur' ? 'background:var(--accent);color:#fff' : 'background:var(--bg-secondary);color:var(--text-muted);border:1px solid var(--border)' ?>">
                Fournisseurs
            </a>
        </div>
    </div>
    ...
    <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:12px;padding:16px 20px;margin-bottom:16px;display:flex;justify-content:space-between;align-items:center">
        <div>
            <div style="font-size:13px;font-weight:700"><?= e($tiers_courant['nom']) ?></div>
            <div style="font-size:13px;color:var(--text-muted);margin-top:2px">
                Période : <?= date('d/m/Y', strtotime($date_debut)) ?> → <?= date('d/m/Y', strtotime($date_fin)) ?>
            </div>
        </div>
        <div style="text-align:right">
            <div style="font-size:14px;text-transform:uppercase;letter-spacing:0.8px;color:var(--text-muted)">Solde ouverture</div>
            <div style="font-family:monospace;font-size:13px;font-weight:700;color:<?= $solde_ouverture > 0 ? '#dc2626' : ($solde_ouverture < 0 ? '#1f6e4e' : 'var(--text-muted)') ?>">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:14px">
            <thead>
                <tr style="border-bottom:2px solid var(--border);background:var(--bg-secondary)">
                    <th style="padding:11px 16px;text-align:left;font-weight:600;font-size:13px;text-transform:uppercase;letter-spacing:0.5px;background:#f1f5f9;color:#4a554f">Date</th>
                    <th style="padding:11px 16px;text-align:left;font-weight:600;font-size:13px;text-transform:uppercase;letter-spacing:0.5px;background:#f1f5f9;color:#4a554f">N° Pièce</th>
                    <th style="padding:11px 16px;text-align:left;font-weight:600;font-size:13px;text-transform:uppercase;letter-spacing:0.5px;background:#f1f5f9;color:#4a554f">Journal</th>
                    <th style="padding:11px 16px;text-align:left;font-weight:600;font-size:13px;text-transform:uppercase;letter-spacing:0.5px;background:#f1f5f9;color:#4a554f">Libellé</th>
                    <th style="padding:11px 16px;text-align:center;font-weight:600;font-size:13px;text-transform:uppercase;letter-spacing:0.5px;background:#f1f5f9;color:#4a554f">Ltr</th>
                    <th style="padding:11px 16px;text-align:right;font-weight:600;font-size:13px;text-transform:uppercase;letter-spacing:0.5px;background:#f1f5f9;color:#4a554f">Débit</th>
                    <th style="padding:11px 16px;text-align:right;font-weight:600;font-size:13px;text-transform:uppercase;letter-spacing:0.5px;background:#f1f5f9;color:#4a554f">Crédit</th>
                    <th style="padding:11px 16px;text-align:right;font-weight:600;font-size:13px;text-transform:uppercase;letter-spacing:0.5px;background:#f1f5f9;color:#4a554f">Solde cumulé</th>
                </tr>
                <!-- Solde ouverture -->
                <?php if($solde_ouverture != 0): ?>
                <tr style="background:rgba(31,110,78,0.04);border-bottom:1px solid var(--border)">
                    <td colspan="5" style="padding:8px 16px;font-size:13px;color:var(--text-muted);font-style:italic">Solde au <?= date('d/m/Y', strtotime($date_debut)) ?></td>
                    <td style="padding:8px 16px;text-align:right;font-family:monospace;font-size:13px"><?= $solde_ouverture > 0 ? formatMontant($solde_ouverture) : '—' ?></td>
                    <td style="padding:8px 16px;text-align:right;font-family:monospace;font-size:13px"><?= $solde_ouverture < 0 ? formatMontant(abs($solde_ouverture)) : '—' ?></td>
                    <td style="padding:8px 16px;text-align:right;font-family:monospace;font-size:13px;font-weight:600"><?= formatMontant(abs($solde_ouverture)) ?> <?= $solde_ouverture > 0 ? 'D' : 'C' ?></td>
                </tr>
                <?php endif; ?>
            </thead>
            <tbody>
            ...
            <tr style="border-bottom:1px solid var(--border)">
                ...
                <td style="padding:10px 16px;font-size:13px;color:#2563eb"><?= e($m['numero_piece'] ?? '—') ?></td>
                <td style="padding:10px 16px">
                    <span style="font-size:14px;font-weight:600;background:var(--bg-secondary);border:1px solid var(--border);border-radius:4px;padding:2px 6px"><?= e($m['journal']) ?></span>
                </td>
                <td style="padding:10px 16px;max-width:280px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><?= e($m['libelle']) ?></td>
                <td style="padding:10px 16px;text-align:center;font-size:14px;font-weight:700;color:var(--text-muted)"><?= e($m['code_lettrage'] ?? '') ?></td>
            ...
            <tfoot>
                <tr style="border-top:2px solid var(--border);background:var(--bg-secondary)">
                    <td colspan="5" style="padding:11px 16px;font-weight:700;font-size:14px">TOTAL PÉRIODE</td>
    ...
    <div class="card" style="text-align:center;padding:48px;color:var(--text-muted)">
        ...
        <p style="font-size:14px;margin:0">Sélectionnez un tiers pour afficher son livre auxiliaire.</p>
    </div>
    <?php endif; ?>
</div>
