...
<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:24px">
    <div class="card" style="padding:16px 20px">
        <div style="font-size:17px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:var(--text-muted);margin-bottom:6px">Total créances</div>
        <div style="font-size:20px;font-weight:700;color:#dc2626;font-family:monospace"><?= formatMontant($total_creances) ?></div>
    </div>
    <div class="card" style="padding:16px 20px">
        <div style="font-size:17px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:var(--text-muted);margin-bottom:6px">> 30 jours</div>
        <div style="font-size:20px;font-weight:700;color:#f59e0b"><?= $nb_retard_30 ?> client<?= $nb_retard_30>1?'s':'' ?></div>
    </div>
    <div class="card" style="padding:16px 20px">
        <div style="font-size:17px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:var(--text-muted);margin-bottom:6px">> 60 jours</div>
...
<div class="card" style="padding:0;overflow:hidden;margin-bottom:24px">
    <div style="padding:16px 20px;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between">
        <div style="font-weight:700;font-size:18px">Créances en retard</div>
        <div style="font-size:16px;color:var(--text-muted)"><?= count($creances) ?> client<?= count($creances)>1?'s':'' ?></div>
    </div>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:16px">
        ...
        <tr style="border-bottom:1px solid var(--border)" id="row-tiers-<?= $c['tiers_id'] ?>">
            <td style="padding:12px 16px">
                ...
                <div style="font-size:18px;color:var(--text-muted);margin-top:2px">
                    <?= e($c['telephone'] ?? '') ?><?= $c['telephone']&&$c['email']?' · ':'' ?><?= e($c['email'] ?? '') ?>
                </div>
                <?php endif; ?>
            </td>
            <td style="padding:12px 16px;text-align:right;font-family:monospace;font-weight:700;color:#dc2626;font-size:18px">
                <?= formatMontant($c['solde']) ?>
            </td>
            <td style="padding:12px 16px;text-align:center">
                <span style="display:inline-block;padding:4px 12px;border-radius:20px;font-size:18px;font-weight:700;background:<?= $couleur ?>22;color:<?= $couleur ?>">
                    <?= $jr ?> j
                </span>
            </td>
            <td style="padding:12px 16px;text-align:center">
                <?php if($niveau_actuel): ?>
                <span style="display:inline-block;padding:3px 10px;border-radius:20px;font-size:18px;font-weight:600;background:<?= $niveaux[$niveau_actuel]['color'] ?>22;color:<?= $niveaux[$niveau_actuel]['color'] ?>">
                    <?= $niveaux[$niveau_actuel]['label'] ?>
                </span>
                <?php else: ?>
                <span style="color:var(--text-muted);font-size:18px">Pas encore relancé</span>
                <?php endif; ?>
            </td>
            <td style="padding:12px 16px;text-align:center">
                <button onclick="ouvrirRelance(<?= $c['tiers_id'] ?>, '<?= e(addslashes($c['tiers_nom'])) ?>', <?= $c['solde'] ?>, <?= $niveau_actuel+1 ?>, '<?= e(addslashes($c['email'] ?? '')) ?>')"
                        style="padding:5px 12px;border-radius:8px;background:#2563eb;color:#fff;border:none;cursor:pointer;font-size:18px;font-weight:600;margin-right:6px">
                    ✉ Relancer
                </button>
...
<div class="card" style="padding:0;overflow:hidden">
    <div style="padding:16px 20px;border-bottom:1px solid var(--border);font-weight:700;font-size:18px">Historique des relances</div>
    <table style="width:100%;border-collapse:collapse;font-size:16px">
        <thead>
...
<div id="modalRelance" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9000;display:none;align-items:center;justify-content:center">
    <div style="background:#fff;border-radius:16px;padding:28px;width:480px;max-width:95vw">
        <div style="font-size:17px;font-weight:700;margin-bottom:20px">Enregistrer une relance</div>
        <div style="margin-bottom:14px">
            <label style="font-size:18px;font-weight:600;color:var(--text-muted);display:block;margin-bottom:5px">Client</label>
            <div id="relance_nom" style="font-weight:600;font-size:18px"></div>
        </div>
        <div style="margin-bottom:14px">
            <label style="font-size:18px;font-weight:600;color:var(--text-muted);display:block;margin-bottom:5px">Niveau de relance</label>
            <select id="relance_niveau" style="width:100%;padding:9px 12px;border-radius:8px;border:1px solid var(--border);font-size:17px">
        ...
        <div style="margin-bottom:14px">
            <label style="font-size:18px;font-weight:600;color:var(--text-muted);display:block;margin-bottom:5px">Notes</label>
            <textarea id="relance_notes" rows="3" style="width:100%;padding:9px 12px;border-radius:8px;border:1px solid var(--border);font-size:17px;resize:vertical" placeholder="Canal utilisé, réponse obtenue..."></textarea>
        </div>
        <div id="relance_email_row" style="margin-bottom:14px;padding:10px 14px;background:#eff6ff;border-radius:8px;font-size:16px;color:#1d4ed8;display:none">
            📧 Un email sera envoyé à : <strong id="relance_email_addr"></strong>
        </div>
        <div id="relance_msg" style="display:none;padding:10px 14px;border-radius:8px;font-size:16px;margin-bottom:10px"></div>
        <div style="display:flex;gap:10px;justify-content:flex-end">
            <button onclick="fermerModal()" style="padding:9px 20px;border-radius:8px;border:1px solid var(--border);background:none;cursor:pointer;font-size:17px">Annuler</button>
            <button id="btn_envoyer_email" onclick="confirmerRelanceEmail()" style="padding:9px 20px;border-radius:8px;background:#2563eb22;color:#2563eb;border:1px solid #2563eb44;cursor:pointer;font-size:17px;font-weight:600;display:none">
                📧 Envoyer par email
            </button>
            <button onclick="confirmerRelance()" style="padding:9px 20px;border-radius:8px;background:#2563eb;color:#fff;border:none;cursor:pointer;font-size:17px;font-weight:600">Enregistrer</button>
        </div>
    </div>
</div>
